...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<?> tratarMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex, WebRequest request){
		...
		String detalhe = String.format("O parâmetro da URL '%s' recebeu o valor '%s', "
				+ "que é inválido. Corrija e informe um valor compatível com o tipo %s.", ex.getName(), ex.getValue(), ex.getRequiredType().getSimpleName());
...
//	}
	 ...
	 * {
	 *     "status": 400,
	 *     "type": "http://algafood.com.br/recurso-em-uso",
	...
	private ResponseEntity<Object> tratarValidacaoIterna(Exception ex, BindingResult bindingResult, HttpHeaders headers,
			HttpStatus status, WebRequest request){		
		List<ProblemaCampo> camposProblema = bindingResult.getAllErrors().stream()
				.map(erro -> {
					...
					return new ProblemaCampo(campo, message);
				})					
	...
	private ResponseEntity<Object> tratarInvalidFormatException(InvalidFormatException ex, 
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		...
		Problema problema = criarProblemaBuilder(status, problemaTipo, detalhe)
				.mensagem(MSG_ERRO_GENERICO_USUARIO_FINAL)
				.build();
		
		return handleExceptionInternal(ex, problema, headers, status, request);
	}
	
	private ResponseEntity<Object> tratarIgnoredPropertyException(PropertyBindingException ex, 
			HttpHeaders headers, HttpStatus status, WebRequest request){
