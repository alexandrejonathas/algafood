...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<?> tratarEntidadeNaoEncontradaException(EntidadeNaoEncontradaException ex,
		...
		Problem problema = criarProblemaBuilder(status, problemaTipo, ex.getMessage()).build();
	...
	public ResponseEntity<?> tratarEntidadeEmUsoException(EntidadeEmUsoException ex, WebRequest request) {
		...
		Problem problema = criarProblemaBuilder(status, problemaTipo, detalhe).message(detalhe).build();
		return handleExceptionInternal(ex, problema, new HttpHeaders(), status, request);
	}
	...
	public ResponseEntity<?> tratarMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex,
		...
		Problem problema = criarProblemaBuilder(status, problemaTipo, detalhe).build();
		return handleExceptionInternal(ex, problema, new HttpHeaders(), status, request);
	}
...
//	}
	...
	private ResponseEntity<Object> handleValidationInternal(Exception ex, BindingResult bindingResult,
		...
		List<Problem.Object> objects = bindingResult.getAllErrors().stream().map(erro -> {
			...
			// Verifica se o erro é uma instancia de fielderror para atribuir o nome do
			// campo com erro
			...
			return new Problem.Object(campo, message);
		...
		Problem problema = criarProblemaBuilder(status, problemaTipo, detalhe).message(detalhe).objects(objects)
				.build();
		return super.handleExceptionInternal(ex, problema, headers, status, request);
	}

	private Problem.ProblemBuilder criarProblemaBuilder(HttpStatus status, ProblemaTipo problemaTipo, String detalhe) {
		return Problem.builder().status(status.value()).type(problemaTipo.getUri()).title(problemaTipo.getTitulo())
